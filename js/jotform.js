...
var JotForm = {
    ...
    init: function (callback) {
        var ready = function () {
            try {
                ...
                this.sessionID = ('session' in document.get) && (document.get.session.length > 0) && JotForm.validateSessionID(document.get.session) ? document.get.session : false;
            ...
            document.dispatchEvent(event);
            if (window.self !== window.top && JotForm.isPaymentSelected()) { // Payment total counter observer doesn't work for embedded forms. Recalculate money if form is embedded form.
    ...
    */
    ...
    handleIFrameHeight: function () {
        ...
        if (isEmbedForm && form.getWidth() === 0) {
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
                    ...
                    if (
                        ...
                        if (productItem.querySelector('.form-checkbox')) {
                            ...
                            if (
                                ...
                                $$('.form-product-input').forEach(function(item) {
    ...
    getMatrixValues: function(questionID, operator) {
        var matrix = document.getElementById("matrix_" + questionID);
        var desktopVersion = matrix.querySelector('.forDesktop');
        var mobileVersion = matrix.querySelector('.forMobile');
        var dataType = matrix.getAttribute('data-type');
        var currentVersion = 'desktop';
        if (desktopVersion && mobileVersion && desktopVersion.getStyle('display') === 'none') {
            currentVersion = 'mobile';
        }
        var checkedValues = function (value) {
            var yesNoID = value.id;
            var yesNoValue = $$('label[for="' + yesNoID + '"]');
            var yesNoValueText = Array.isArray(yesNoValue) && yesNoValue.length > 0 ? yesNoValue[0].innerText : value.checked;
        ...
        switch(true) {
            case dataType === 'Yes No' && (operator == 'contains' || operator == 'notContains'):
                ...
                return $$(selector).map(function(value) {
                ...
                case 'inline':
                    ...
                    if (data.indexOf('|') > -1) {
                        ...
                        $$(selector).each(function (el) {
                            if (!el.value.empty() && ($('draftID') || JotForm.isVisible(el)) && (!(['checkbox', 'radio'].indexOf(el.getAttribute('type')) > -1) || el.checked)) {
                                valArr.push(el.value);
                            }
    ...
    */
    ...
    checkEmbed: function () {
        ...
        if (window !== window.top) {
            var embedURLInput = document.createElement('input');
            embedURLInput.type = 'hidden';
            embedURLInput.name = 'embedUrl';
            embedURLInput.value = document.referrer;
            form.appendChild(embedURLInput);
            if (JotForm.debug) {
   ...
   */
    ...
    highLightLines: function () {
        ...
        formLineList.forEach((formLine, index) => {
                ...
                document.querySelectorAll('.form-textarea, .form-textbox').forEach(function (el) {
                    ...
                    if (el.classList.contains("form-textarea") && "nicEditors" in window) { //"nicEditors" in window added for somehow if this check fails, do not give errors
                        document.querySelectorAll("#cid_" + el.id.split("_")[1] + " > div:nth-child(1)").forEach(function (tmpel) {
                            if (tmpel.attributes.unselectable.value == "on") {
                                var [toolbarEl, containerEl] = Array.from(document.querySelectorAll("#cid_" + el.id.split("_")[1] + " > div"));
                                // hide toolbar
    ...
    initTextboxValidation: function () {
        $$('.form-textbox[data-minlength]').each(function (item) {
            item.validateTextboxMinsize = function () {
                ...
                var isRequired = item.getAttribute('required') !== null;
        ...
        if (pages.length > 0) {
            ...
            if (last) {
                ...
                if (mergeWithSubmit) {
                    ...
                    // reorder buttons
                    var buttonOrder = ['form-pagebreak-back-container', 'form-submit-preview', 'form-submit-print', 'form-submit-reset', 'paypal-submit-button-wrapper', 'form-sacl-button', 'form-submit-button', '.useJotformSign-button', 'form-pagebreak-next-container'];
                    buttonOrder.forEach(btnClass => {
                        [...targetSubmit.children].forEach(child => {
                            if (child.classList.contains(btnClass)) {
    ...
    isInputAlwaysHidden: function (input) {
        ...
        var addressLineWrapper = input.up('.form-address-line-wrapper') || input.up('.form-address-line');
    ...
    showFormSection: function (section, withoutFocus) {
        ...
        if (firstLabel && !firstLabel.closest('.form-section-closed') && !withoutFocus) {
  ...
  handleSignSignatureInputs: function() {
            ...
            Array.prototype.forEach.call(allValidateClasses, function(input) {
                if (JotForm.payment && input.up('.form-line')) { //b#486482 only run on first payment input as that will iterate over all of the others
                    ...
                    if (dataType == "control_" + JotForm.payment) {
                        ...
                        if (input.getAttribute('data-inputname') && !input.getAttribute('data-inputname').match('cc_') && !isFirstProduct) {
                            return;
                        }
                ...
                if (
                    ...
                    if (JotForm.pricingInformations && JotForm.pricingInformations.general) {
                        const paymentSummary = {
                            ...
                            shipping_discounted: JotForm.pricingInformations.general.shippingDiscountVal || 0,
                            tax: JotForm.pricingInformations.general.tax_total || 0,
                            tax_discounted: (JotForm.pricingInformations.general.taxTotalWithoutDiscount - JotForm.pricingInformations.general.tax_total) || 0,
                            subtotal: JotForm.pricingInformations.general.item_total || 0,
                            total: JotForm.pricingInformations.general.net_amount || 0,
    ...
    */
    ...
    validateSessionID: function (session) {
        ...
        var sessionIDList = ['StripeCheckout-', 'Payfast-', 'CashApp-', 'Sensepass-', 'JF-S4L'];
